...
from utils import divide_0div0, testing_measure
...
class TestMethodologyFunctionCorrectness:
    ...
    def test_trivial(self):
        ...
        m_0 = 20
        m_1 = 20
        easy_p_values = [0]*m_0+[1]*m_1 # Super convenient p values
        random.shuffle(easy_p_values
        for method = [MultiTest.BonferroniMethod, MultiTest.BHMethod, MultiTest.HochbergMethod]:
            for alpha in [0.01, 0.05, 0.1, 0.2, 0.5, 0.9]:
                output = method(easy_p_values, alpha)
                assert (np.abs(output-easy_p_values) < 1e-9)).all(),
                       f'Method {method} with alpha {alpha} gives unexpected result'
...
class TestUtilFunctionCorrectness:
    ...
    def test_matching(self):
        ...
        result = testing_measure(ground_truth, test_result, criteria=('power', 'type1', 'type2', 'fdr'))
    ...
    def test_matching_2(self):
        ...
        expected_result['fdr'] = np.array([0.5, 0.5, 0, 1])
        for criterion in ('power', 'type1', 'type2', 'fdr'):
            assert (abs(result[criterion]-expected_result[criterion]) < 1e-9).all(),
                   f'Power should be {expected_result[criterion]}, but got {result[criterion]}'
